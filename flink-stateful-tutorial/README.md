...
If everything runs correctly, the jar is located under `target/flink-stateful-tutorial-1.13.2-csadh1.5.0.0.jar`.
...
flink run -d -p 2 -ys 2 -ynm DataGenerator -c com.cloudera.streaming.examples.flink.KafkaDataGeneratorJob target/flink-stateful-tutorial-1.13.2-csadh1.5.0.0.jar config/job.properties
```
...
```shell
flink run -d -p 8 -ys 4 -ytm 1500 -ynm TransactionProcessor target/flink-stateful-tutorial-1.13.2-csadh1.5.0.0.jar config/job.properties
```
...
```shell
flink run -d -p 8 -ys 4 -ytm 1500 -ynm TransactionProcessor -s hdfs://savepointpath target/flink-stateful-tutorial-1.13.2-csadh1.5.0.0.jar config/job.properties
```
